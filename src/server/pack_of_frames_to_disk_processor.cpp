...
void PackOfFramesToDiskProcessor::onNewFrame(PackOfFrames& framePacks, int frameNo) {
  ...
  for (auto &entry : framePacks) { 
    std::string path = entry.first + "_depth_" + std::to_string(frameNo) + ".png";
    ...
    path = entry.first + "_rgb_" + std::to_string(frameNo) + ".png";
    ...
    path = entry.first + "_ir_" + std::to_string(frameNo) + ".png";
